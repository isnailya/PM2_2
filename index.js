class Deposit {
    constructor (startamount,monthamount,depositterm,valuta) {
    ...
    getpercent(){
        return (this.monthamount*100)/(this.startamount);
    }
}
...
class Calculator {
    ...
    findBestBank() {
       // let a= typeof this.list[0];
             ...
             {
                if(this.list[i].incomeType>=this.dep.percent && this.list[i].currency==this.dep.valuta && this.list[i].sumMin<=this.dep.startamount && this.list[i].termMin<=this.dep.depositterm && this.list[i].termMax>=this.dep.depositterm) {
                   //let y = typeof list[i];
    ...
    run(){
         ...
         if (valuta=='')
        {
            ...
            for (let i=0;i<list.length;i++) {
                let bankProduct = new BankProduct( list[i].bankName,  list[i].investName,  list[i].currency,list[i].incomeType , list[i].sumMin, list[i].sumMax, list[i].termMin, list[i].termMax, list[i].canDeposit);
                arrayBankProduct.push(bankProduct);
            } 
            let calculator = new Calculator(arrayBankProduct,deposit);
            let newarray=[];
            let calcres = [];
